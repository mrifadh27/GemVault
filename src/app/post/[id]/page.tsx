...
import {
  ...
  CheckCircle, MapPin, Award, MoreHorizontal, Trash2, Tag, Eye
} from 'lucide-react';
...
export default function PostPage() {
  ...
  const handleLike = async () => {
    ...
    try {
      await toggleLike.mutateAsync({ postId: post.id, liked: isLiked });
    } catch {
      setLiked(isLiked);
  ...
  const handleMarkSold = async () => {
    try {
      await markSold.mutateAsync({ postId: post.id, sold: !post.is_sold });
      toast(post.is_sold ? 'Marked as available' : 'Marked as sold', 'success');
      ...
      <main className="pt-14 pb-20 sm:pb-8 max-w-2xl mx-auto">
        ...
        <div className="flex items-center justify-between px-3 py-2">
          ...
          <div className="flex items-center gap-2">
            {post.is_sold && (
              <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-red-500/20 text-red-400 border border-red-500/30">SOLD</span>
            )}
            {isOwner && (
          ...
          {images.length > 1 && (
            <>
              ...
              <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1">
                {images.map((_, i) => (
                  <button key={i} onClick={() => setImgIndex(i)} className={cn('w-1.5 h-1.5 rounded-full transition-all', i === imgIndex ? 'bg-white w-3' : 'bg-white/40')} />
                ))}
              </div>
            </>
          )}
        ...
        <div className="px-3 pt-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <button onClick={() => !post.is_sold && setShowDM(true)} className="flex items-center gap-1.5" disabled={post.is_sold}>
              <MessageCircle className={cn('w-6 h-6', post.is_sold ? 'text-[#333]' : 'text-ivory-muted hover:text-gold transition-colors')} />
            </button>
        ...
        <div className="px-3 py-3">
          ...
          {post.price ? (
            <div className="mb-3">
              ...
              {post.is_price_negotiable && <span className="text-sm text-ivory-muted ml-2">· Negotiable</span>}
            </div>
          ) : (
          ...
          {post.certification && post.certification !== 'None' && post.certification_number && (
            <p className="text-xs text-gold/80 mb-3">
              📜 {post.certification} Certificate #{post.certification_number}
            </p>
          )}
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {post.tags.map(tag => (
                <Link key={tag} href={`/explore?tag=${tag}`} className="text-xs text-gold/70 hover:text-gold transition-colors">
          ...
          {!isOwner && !post.is_sold && (
            <button onClick={() => setShowDM(true)} className="dm-btn w-full justify-center py-3.5 rounded-2xl text-sm font-bold mb-4">
              <MessageCircle className="w-5 h-5" />
              Need DM · Message Seller
            </button>
          )}
          ...
          <div className="card p-4 flex items-center gap-3 mt-2">
            ...
            <Link href={`/profile/${seller.username}`} className="btn-outline text-xs px-3 py-1.5 rounded-full">
              View Profile
            </Link>
          </div>
